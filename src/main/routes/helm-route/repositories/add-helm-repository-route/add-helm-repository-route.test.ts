...
describe("add-helm-repository-route", () => {
  let router: Router;
  ...
  describe("when request for adding helm repository", () => {
    ...
    it("executes Helm command for adding repository", () => {
      expect(execFileMock).toHaveBeenCalledWith(
        "some-helm-cli-path",
        ["repo", "add", "some-repository-name", "some-repository-url"],
        undefined,
      );
    });

    it("when Helm command resolves with success, resolves with success", async () => {
      await execFileMock.resolve({ stdout: '"some-repository-name" has been added to your repositories', stderr: null });
